...
def safe_skew(x: np.ndarray) -> float:
    """
    Population skewness (3rd standardized moment). Returns 0 for small/constant arrays.
    """
...
def safe_kurtosis(x: np.ndarray) -> float:
    """
    Excess kurtosis (4th standardized moment - 3). Returns 0 for small/constant arrays.
    """
    x = np.asarray(x, dtype=float)
    if x.size < 4:
        return 0.0
